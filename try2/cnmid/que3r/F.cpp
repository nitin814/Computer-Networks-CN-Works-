...
void * func (void * args)
{
    rsfd = socket(AF_INET , SOCK_DGRAM , 0);
    if (rsfd==-1)
    {
    ...
    while (1)
    {
        ...
        clientports.insert(clientaddress.sin_port);
    ...
    while (1)
    {
        ...
        for (int i=0;i<cnt;i++)
        {
            ...
            else if (i==1 && (pfd[1].revents & POLLIN))
            {
                ...
                string str = str1;
                str += " has joined the F Process .";
                
                for (auto i : clientports)
                {
                    struct sockaddr_in clientsaddr;
                    clientsaddr.sin_family = AF_INET;
                    clientsaddr.sin_addr.s_addr = inet_addr("127.0.0.0");
                    cout<<"sending to client no : "<<i<<" the message : "<<str<<endl;
                    clientsaddr.sin_port = htons(i);
                    sendto(rsfd , str.c_str() , strlen(str.c_str())+1 , 0 , (struct sockaddr *)&clientsaddr , sizeof(clientsaddr));
                }
